...
ERROR_MESSAGES = {
    ...
    "name_format"     : "❌ 이름 오류: 이름은 한글 또는 영문 1자 이상 10자 이하로 입력해야 합니다.",
    ...
    "address_length"  : "❌ 주소는 100자 이내로 입력해주세요.",
    "duplicate_phone" : "❌ 중복 오류: 이미 등록된 전화번호입니다.",
    "inpt_keyword"    : "❌ 키워드를 입력해주세요.",
    "invalid_keyword" : "❌ 키워드는 1자 이상 16자 이하로 입력해주세요.",
    "invalid_keyword2": "❌ 한글, 영문, 숫자만 입력해주세요. 특수문자는 사용할 수 없습니다.",
    "invalid_input"   : "❌ 잘못 입력했습니다. 다시 시도하세요.",
    "invalid_number"  : "❌ 유효하지 않은 번호입니다.",
    "input_num_only"  : "❌ 숫자만 입력해주세요.",
...
MESSAGES = {
  ...
  'savedData' : '\n💾 회원 정보가 파일에 저장되었습니다.\n',
  'noData' : '🚫 등록된 회원이 없습니다. 🔙 메인 메뉴로 돌아갑니다. ', 
  'not_found': "🚫 검색된 목록이 없습니다.",
  'getActionGo2Main' : "🔙 메인 메뉴로 돌아가려면 '<' 를 입력하세요: " ,
  'getActionAfterList' : "\n📄상세 조회할 번호를 입력하거나, '<' 을 입력해 메인 메뉴로 돌아가세요: ", 
  "getActionAfterView" : "\n회원 목록으로 돌아가시겠습니까? (y. 예, n. 아니오): ",
  ...
  'inputNumber4EditOrBack2Menu' : '\n✏️ 수정할 번호를 입력하거나,  🔙 메인 메뉴로 돌아가려면 '<'을 입력하세요: ',
  'getActionAfterEditOK': "\n🔙 메인 메뉴로 돌아가려면 '<'을 입력하거나, 계속 수정하려면 'e'를 입력하세요: ", 
  'inputNumber4DelOrBack2Menu' : '\n✏️ 삭제할 번호를 입력하거나,  🔙 메인 메뉴로 돌아가려면 '<'을 입력하세요: ',
  'getActionAfterDelOK': "\n🔙 메인 메뉴로 돌아가려면 '<'을 입력하거나, 계속 삭제하려면 'd'를 입력하세요: ", 
  "delete_confirm"  : "정말로 삭제하시겠습니까? (y. 삭제, n. 취소하고 메인 메뉴로 돌아가기): ",  
  "exit_confirm"    : "\n📄 정말로 종료하시겠습니까? (y. 예, n. 아니오): ",
  'mainMenu' : '''
==================================================
      [ 회원관리 프로그램 메인 메뉴 ]  
--------------------------------------------------            
...
class MemberManager:
    ...
    def print_menu_title(self, menu_no)->str:
        ''' 메뉴 타이틀 출력하기 '''
        if '1' <= menu_no < '5': # 메뉴번호가 1 ~ 5이면
            print()  # 빈줄 한 줄 출력
            title_len = 60 # 양쪽 여백 20 칸 
    ...
    def save_members(self):
        ...
        print(f"{MESSAGES['savedData']}") # logging message print
    ...
    def view_detail(self, view_no:int) -> None:
        ...
        while True:          
          ...
          if action_no == '1': # 1. 메뉴로 돌아가기 
              return 
          elif action_no == '2': # 2. 아니오 선택시 다시 입력받기 
              continue
          else : # 그외 값 입력시 
    ...
    def list_members(self, menu_no:str)->str:
      ...
      while True:
        ...
        try:
          ...
          self.view_detail(++view_no) # index는 0 인데 출력을 1부터 했으니 +1해서 전달하기 
    ...
    def input_name(self, name="")-> str:
      ...
      placeholder = "(한영 한글자 이상 10자 이내로 입력하세요)."
      while True:
        if name: 
            placeholder = f"현재 이름: {name}를 수정할 이름을 10자 이내로 입력하세요."
        print(f"\n ⌨️ {placeholder}")
        name = input(f"👤 이름* : ") # 이름 입력받기
        if not name: # 입력값이 없는 경우 에러 메세지 출력
            self.print_error("name_empty")
            continue
        if not re.fullmatch(r'[가-힣a-zA-Z0-9]{1,10}', name): # 입력값이 한영자 10자이가 아닌 경우 에러 처리
    ...
    def input_relation(self, relation:str="")->str:
      ...
      while True:
        ...
        valid_relations = {'1', '2', '3'} # 입력 가능한 값 지정 
        if relation not in valid_relations: # 입력 가능한 값이 아닌 경우 에러 메세지 출력하고
                ...
                if 1 <= selected_num <= len(self.search_results): # 출력한 범위안에 있으면                 
                  ...
                  while True:                   
                    ...
                    if del_yn.lower() == 'y': # 삭제하기                   
                        ...
                        # 다음 액션 받기
                        while True: 
                          action = input(MESSAGES['actionAfterDelOK']).strip() # 입력 받고 공백제거
                          if action.lower() == Command.RETURN_TO_MAIN.value: # < 입력받은 경우
                              return
                          elif action_no.lower() == Command.EDIT_AGAIN.value : # 다시 수정하기 선택시 
                            break
