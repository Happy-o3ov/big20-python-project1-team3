...
class MemberController:
    ...
    def run(self):
        ...
        while True:
            ...
            if choice == 1:
                self.view.show_members(self.model.get_members())
                ...
                if self.model.is_phone_duplicate(info["phone"]):
                    ...
                    continue
                self.model.insert_member(name, info)
                ...
                if not members:
                    ...
                    continue
                self.view.show_members({name: members})
                idx = self.view.input_index("수정")
                if idx < 0 or idx >= len(members):
                    self.view.show_message("잘못된 번호입니다.")
                    continue
                _, new_info = self.view.input_member_info()
                if not self.model.is_valid(new_info):
                    self.view.show_message("입력값이 유효하지 않습니다.")
                    continue
                if self.model.is_phone_duplicate(new_info["phone"]) and new_info["phone"] != members[idx]["phone"]:
                    self.view.show_message("이미 등록된 전화번호입니다.")
                    continue
                self.model.update_member(name, idx, new_info)
                ...
                if self.view.confirm("정말로 삭제하시겠습니까?"):
                    self.model.delete_member(name, idx)
