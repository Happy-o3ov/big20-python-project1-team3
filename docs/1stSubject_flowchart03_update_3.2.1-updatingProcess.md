...
edit_info
    ...
    %% duplJudge -- False --> updateInfo@{ shape: lin-cyl, label: "회원 정보 수정 저장" };
      ...
      updateInfo --> actionAfterEditOK@{ shape: sl-rect, label: "'<' Main Menu으로 돌아가기 <br> 'c' 수정 계속"};
      actionAfterEditOK -- '<' 입력시 --> Menu((Main Menu 출력));
      actionAfterEditOK -- 'c' 입력시 --> Update(( 3.수정 프로세스));
      actionAfterEditOK -- 그외 --> errorInputPrint2@{ shape: doc, label: "잘못 입력하셨습니다. <br> 다시 입력하세요"};
                          errorInputPrint2 --> actionAfterEditOK;        
